...
def set_missing_time(filename,raw_file):
    ...
    no_missing_1 = no_missing
    no_missing = no_missing.drop("Total Time Spent on Website",axis=1)
    missing = missing.drop("Total Time Spent on Website",axis = 1)

    return no_missing,missing,y_no_missing,x,raw,no_missing_1

def estimate(no_missing,missing,no_missing_1):
    clf = KMeans()
    clf.fit(no_missing)
    ...
    i = 0
    for item in label:
        sums[str(item)] += no_missing_1["Total Time Spent on Website"][i]
        count[str(item)] += 1
        i += 1
    for key in count.keys():
        means[str(key)] = sums[str(key)]/count[str(key)]
    for item in predictedtime:
        result.append(means[str(item)])
    return result
...
def sub_data(raw_data,missing_mean,missing_index):
    c = 0
    i = 0
    for item in missing_index:
        raw_data.loc[item, 'Total Time Spent on Website'] = missing_mean[c]
        c += 1
    """for item in no_missing_index:
        raw_data.loc[item,'Total Time Spent on Website'] = no_missing_mean[i]
        i += 1"""
...
no_missing,missing,y_no_missing,raw_set,Raw,no_missing_1 = set_missing_time("Missing1.csv","public_data1.csv")
X_no_missing = encoding(no_missing)
X_missing = encoding(missing)
missing_index = list(missing.index)
no_missing_index = list(no_missing.index)
missing_mean = estimate(X_no_missing,X_missing,no_missing_1)
Estimate_set,label = sub_data(raw_set,missing_mean,missing_index)
